    ...
    return {
        ...
        set: (key, value) => {
            if (!key || value === undefined && typeof key !== 'object')
                throw new Error("Modo de uso: \"key\", \"value\"");
        ...
        delete: (key) => {
            if (!key)
                throw new Error("Obrigatório o uso de uma chave: \"key\"");
            if (typeof key !== "string")
                throw new Error("String não identificada.");
...
function base64(param) {
    ...
    return {
        ...
        get: (key) => {
            ...
            Object.values(param.get).map((value, index) => ({ [keys[index]]: Buffer.from(value, 'base64').toString("utf-8") })).map((value) => { Object.assign(obj, value); });
            if (!key)
                return obj;
            if (!param.get[key])
                return;
        ...
        set: (key, value) => {
            ...
            if (value !== undefined && typeof value !== 'string')
                throw new Error("Só passe \"string\" no \"value\".");
            if (typeof key == 'object') {
                let values = [];
...
function modify(dir, space, codifyType) {
    ...
    if (!["utf-8", "base64"].includes(codifyType) && codifyType)
        throw new Error("Os tipos de valodação de dados são: \"utf-8\" ou \"base64\"");
    ...
    */
    constructor(space, codifyType) {
        if (typeof space !== "number" && space)
            throw new Error(`Tenho certeza de que \"${space}\" não é um numero.`);
        if (typeof codifyType !== "string" && codifyType && codifyType == "")
            throw new Error(`O tipo: \"${codifyType}\" não é aceito, procure algo como (\"base64\" ).`);
        /**
